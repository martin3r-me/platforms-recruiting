<div class="min-h-screen bg-[var(--ui-surface)] py-8 px-4">
    <div class="max-w-2xl mx-auto">
        <div class="text-center mb-8">
            <h1 class="text-2xl font-semibold text-[var(--ui-secondary)]">Ihre Verträge</h1>
        ...
        @elseif($state === 'invalid')
            <div class="p-6 bg-red-50 border border-red-200 rounded-lg text-center">
                ...
                <p class="text-sm text-red-700">Dieser Link ist nicht gültig oder wurde widerrufen. Bitte wenden Sie sich an den Absender.</p>
            </div>
        @elseif($state === 'expired')
            <div class="p-6 bg-amber-50 border border-amber-200 rounded-lg text-center">
                ...
                <p class="text-sm text-amber-700">Dieser Link ist abgelaufen. Bitte fordern Sie einen neuen Link an.</p>
            </div>
        @elseif($state === 'empty')
            <div class="p-6 bg-[var(--ui-muted-5)] border border-[var(--ui-border)] rounded-lg text-center">
                ...
                <p class="text-sm text-[var(--ui-muted)]">Aktuell sind für Sie keine Verträge zur Unterschrift hinterlegt.</p>
            </div>
        @else
            <div class="space-y-3">
                        ...
                        <div class="flex items-start justify-between gap-4">
                            <div class="flex-1 min-w-0">
                                <h3 class="font-medium text-[var(--ui-secondary)]">
                                    {{ $c['template_name'] }}
                                    @if($c['template_code'])
                                        <span class="text-xs text-[var(--ui-muted)] ml-1">({{ $c['template_code'] }})</span>
                                    @endif
                                </h3>
