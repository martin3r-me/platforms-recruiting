...
<div class="space-y-2 text-sm">
    <div class="flex flex-wrap gap-4">
        <label class="flex items-center gap-2">
            <input type="radio" wire:model.live="escDay" value="vortag" class="border-gray-300"> Vortag
        </label>
        <label class="flex items-center gap-2">
            <input type="radio" wire:model.live="escDay" value="einsatztag" class="border-gray-300"> Einsatztag
        </label>
        <label class="flex items-center gap-2">
            <input type="radio" wire:model.live="escDay" value="datum" class="border-gray-300"> Datum
        </label>
    </div>
    @if ($escDay === 'datum')
        <label class="block text-xs text-gray-600">Eskalation am
            <input type="date" wire:model="escDate" min="{{ now()->toDateString() }}" class="mt-1 w-48 rounded border border-gray-300 px-2 py-1 text-sm">
            <span class="mt-1 block text-gray-500">An diesem Tag laufen alle drei Stufen für <strong>alle</strong> kommenden Einsatztage dieser Veranstaltung.</span>
        </label>
    @endif
    ...
    <div class="flex items-center justify-between text-xs text-gray-500">
        <span>Leer = Standard ({{ $defaults[1] }} / {{ $defaults[2] }} / {{ $defaults[3] }}). Am Einsatztag (oder wenn das Datum auf dem ersten Einsatztag liegt) müssen alle Stufen vor Schichtbeginn liegen.</span>
