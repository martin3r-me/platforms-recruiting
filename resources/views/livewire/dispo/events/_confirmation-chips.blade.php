...
@elseif ($assignment->reminder_sent_at)
    ...
    @endif
    <span class="rounded bg-blue-50 px-1.5 py-0.5 text-xs text-blue-700" title="Gesendet {{ $assignment->reminder_sent_at->format('d.m.Y H:i') }}{{ $assignment->escalation_due_1_at ? ' — eigener Eskalationsplan: ' . $assignment->escalation_due_1_at->format('d.m.Y') . ' · ' . $assignment->escalation_due_1_at->format('H:i') . ' / ' . $assignment->escalation_due_2_at?->format('H:i') . ' / ' . $assignment->escalation_due_3_at?->format('H:i') : '' }}">angeschrieben {{ $assignment->reminder_sent_at->format('d.m. H:i') }}</span>
    @if ($msgStatus === 'failed')
