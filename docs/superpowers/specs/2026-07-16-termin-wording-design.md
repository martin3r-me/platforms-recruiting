...
| Stelle | Änderung |
|---|---|
| `src/Livewire/Public/InterviewBooking.php` | exponiert **komplette, registergerechte Sätze** als public Properties (`$terminGebuchtTitel`, `$terminGebuchtSatz`, `$terminAbsagenLabel`, `$terminAbsagenConfirm`), berechnet via `TerminWort` + `$duzen` (beides liegt im `mount()` vor; nach Buchung/Umbuchung neu berechnen). Bewusster Trade: für diese Stellen wandert das du/Sie-Ternary aus dem Blade in die Komponente — erschlägt Casing (satzinitial groß) und den Attribut-Pitfall zusammen |
| `resources/views/livewire/public/interview-booking.blade.php` | „Termin gebucht!" → `{{ $terminGebuchtTitel }}`; „Dein/Ihr Interview-Termin wurde erfolgreich gebucht." → `{{ $terminGebuchtSatz }}`; Button „Schulung absagen" → `{{ $terminAbsagenLabel }}`; Absage-Confirms (Alpine-`@click`-Attribute Z. 318/398) → vorberechneter Satz via `@js($terminAbsagenConfirm)` (Apostroph-sicher, kein Inline-Ternary im Attribut). **Unverändert:** Cancelled-State (enthält kein Termin-Wort), Warteliste-Texte und Warteliste-Confirm Z. 216 (beziehen sich auf mehrere/unbestimmte Termine) |
| `src/Models/RecApplicant.php` (`renderPublicFormCompletionExtras`) | reicht zusätzlich zu `duzen` den fertigen Bestätigungs-Satz ans Partial. Null-Kette: `booking`/`interview` sind an dieser Stelle garantiert (early return `!$booking?->interview`), **nur `interviewType` kann null sein** → `TerminWort::from($booking->interview->interviewType)` fällt auf „Termin" zurück |
| `resources/views/partials/public-form-completion.blade.php` | „Deine Schulung ist bestätigt!" → vorberechneter Satz `{Possessiv, satzinitial groß} ist bestätigt!` (Casing-Regression vermeiden: heute steht dort großes „Deine"); „Weitere Infos zur Schulung findest du hier:" → „Weitere Infos findest du/finden Sie hier:" (Präpositions-Formen bewusst vermieden — nur die 3 Formen der Wort-Klasse verwenden) |
